       ...
       public OperationResult Create(CreateProduct command)
       {
           ...
           var slug = command.Slug.Slugify();
            var product = new Product(command.Name, command.Code, command.UnitPrice, command.ShortDescription,
               command.MetaDescription, command.Picture
        ...
        public OperationResult Edit(EditProduct command)
        {
            ...
            if (_productRepository.Exists(x => x.Name == command.Name && x.Id != command.Id))
                return operation.Failed(ApplicationMessages.DuplicatedRecord);
            var slug = command.Slug.Slugify();
            product.Edit(command.Name,command.Code,command.UnitPrice,command.ShortDescription,command.MetaDescription,command.Picture
